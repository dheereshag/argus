...
3. **Stage 2: Optical Character Recognition (OCR)** ([`app/services/ocr/`](../app/services/ocr/), [`app/services/pipeline/`](../app/services/pipeline/)):
   ...
   - **Contrast & Resolution Enhancement Fallback**: If the initial OCR extraction pass returns no valid license plate, Argus triggers a fallback pass applying bounded linear upscaling ($2.0\times$ for crops $< 300\text{px}$) and CLAHE (Contrast Limited Adaptive Histogram Equalization, `clipLimit=3.5`, `tileGridSize=(4, 4)`).
   - **Debug Crop Persistence**: Governed by `DEBUG_SAVE_CROPS` (default: `false`) and `DEBUG_CROPS_DIR` (default: `debug_crops`). When enabled, writes the raw vehicle crops (`_raw.jpg`) and contrast-enhanced images (`_enhanced.jpg`) to disk with timestamps and vehicle indices for visual inspection and debugging.
