...
def normalize_candidate_strings(raw_str: str) -> List[str]:
    ...
    for cand in candidates:
        ...
        # 9-character plate: [State 2L][District 2D][Series 1L][Serial 4D]
            ...
            st_corr = STATE_PREFIX_CORRECTIONS.get(st, st)
            dist_corr = "".join(CHAR_TO_DIGIT.get(c, c) for c in dist)
            series_corr = "".join(DIGIT_TO_CHAR.get(c, c) for c in series)
            serial_corr = "".join(CHAR_TO_DIGIT.get(c, c) for c in serial)
            ...
            dist_1 = "".join(CHAR_TO_DIGIT.get(c, c) for c in cand[2:3])
            ser_2 = "".join(DIGIT_TO_CHAR.get(c, c) for c in cand[3:5])
            ser_4 = "".join(CHAR_TO_DIGIT.get(c, c) for c in cand[5:9])
