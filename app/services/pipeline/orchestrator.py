...
def _handle_ineligible(
    detection: DetectionResult, image_bytes: bytes, filename: str, start_time: float
) -> RecognitionResponse:
    fallback = _attempt_zero_vehicle_fallback(detection, image_bytes, filename)
...
def recognize_plate_image(
    ...
    detection = pl.VehicleDetector().detect(image_bytes)
    if not detection.is_eligible:
        return _handle_ineligible(detection, image_bytes, resolved_filename, start_time)
