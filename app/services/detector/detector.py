...
class VehicleDetector:
    ...
    _evaluate_occupancy = staticmethod(evaluate_occupancy)
    ...
    def detect(self, image_input: ImageInput) -> DetectionResult:
        """Detect 4-wheeler vehicles, verify occupancy, and extract vehicle crops."""
        pil_img = load_rgb(image_input)
        h_cnt, vehicles = self._run_detection(pil_img, settings.HUMAN_CONF_THRESH, settings.VEHICLE_CONF_THRESH)
        ok, status = evaluate_occupancy(h_cnt, vehicles)
        return DetectionResult(is_eligible=ok, status=status, human_count=h_cnt, vehicles=self._build_detected_vehicles(vehicles, pil_img))
